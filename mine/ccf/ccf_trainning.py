...
'''
'''
t = train_input.sample(100000)
# param_test1 = {'learning_rate': [0.1, 0.14, 0.18, 0.22, 0.26, 0.30]}  # 0.2
param_test1 = {'n_estimators': range(400, 900, 150)}  # 400
...
gsearch1 = GridSearchCV(
    ...
    param_grid=param_test1, scoring='roc_auc', iid=False, cv=2, verbose=2, n_jobs=3)
...
classifier = GradientBoostingClassifier(learning_rate=0.12, n_estimators=400, max_depth=5,
                                        ...
                                        max_leaf_nodes=12)
classifier.fit(train_input.drop(columns=drop_t_columns).values,
               train_input['target'].values)
...
classifiers = [joblib.load('3m_gdbt.model'), joblib.load('2m_gdbt.model'), joblib.load('leakage_3m_gdbt.model')]
predict_samples = [
    pd.read_csv('3m_samples_2016-07-01~2016-08-01.csv').drop(columns=drop_columns),
    pd.read_csv('2m_samples_2016-07-01~2016-08-01.csv').drop(columns=drop_columns),
    pd.read_csv('leakage_3m_samples_2016-07-01~2016-08-01.csv').drop(columns=drop_columns)
]
...
t['Probability'] = 0.6 * predict_samples[0]['Probability'] + 0.3 * predict_samples[1][
