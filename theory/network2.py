...
class Network(object):
    ...
    def backprop(self, x, y):
        ...
        #根据z的表达式，权重w误差等于偏置b的偏差乘以上一层w对应的输入
    ...
    #我将下面两个convert改为converted， True代表已经处理过
    # 原来的意思太不清楚， 变量应该表示状态，而不是动作
    def total_cost(self, data, lmbda, converted=True):
        """
        lmbda为罚项
        """
    ...
    def accuracy(self, data, converted=True):
...
if __name__ == '__main__':
    ...
    o_train = [vectorized_result(y) for y in y_train]
    o_train = np.array(o_train)
    o_test = [vectorized_result(y) for y in y_test]
    net = Network([784, 30, 10], CrossEntropyCost)
    x_train = x_train.reshape(60000, 784, 1)
    x_test = x_test.reshape(10000, 784, 1)

    # 注意这里输入o_train是处理过的10x1矩阵
    evaluation_cost, evaluation_accuracy, training_cost, training_accuracy = net.SGD(list(zip(x_train, o_train)), 30,
                                                                                     10, 0.5, lmbda=5.0,
