...
'''
positional_encoding 加入位置信息，embedding与 positional_encoding相加前先乘以sqrt(d_model)原因不明
self-attention 通过通过dense，reshape来构造多头,多头合并长度和原来embedding size模型一样。
'''
...
def scaled_dot_product_attention(q, k, v, mask):
    # self attention seq_len_k seq_len_v seq_len_q 相等
    """计算注意力权重。
    q, k, v 必须具有匹配的前置维度。
    ...
    # 将 mask 加入到缩放的张量上。目标是将这些单元归零，因为 softmax 的较大负数输入在输出中接近于零。
    if mask is not None:
...
def train_step(inp, tar):
    ...
    with tf.GradientTape() as tape:
        predictions, _ = sample_transformer(inp, tar_inp,
                                            True,
                                            enc_padding_mask,
                                            combined_mask,
                                            dec_padding_mask)
...
if __name__ == '__main__':
    ...
    # num_layers enc_layers，dec_layers × N值  d_model embedding size(也是MultiHeadAttention 输出的多头注意力合并的维度), dff forword里面全连接的数量
    # pe_input，pe_target位置编码的最大位置，和输入序列长度有关
    sample_transformer = Transformer(
        num_layers=2, d_model=512, num_heads=8, dff=2048,
        input_vocab_size=8500, target_vocab_size=8000,
        pe_input=10000, pe_target=6000)

    temp_input = np.random.randint(100, size=(64, 26))
